	...
	jQuery.fn.html5_video_playlist = function(fn_options){
		...
		self.loadVideo = function(){
			...
			if (self.options.skin == 1){
				self.form_video_show.css({
					'height': self.options.form_height - self.options.form_video_description_height - self.options.form_video_control_height - self.options.form_video_list_height,
					'width': '100%',
					'cursor': 'pointer'
				});
			}else{
			...
			self.form_video_show[0].addEventListener('loadeddata', function(){
				...
				self.form_video_show_overlay.unbind("click").click(function(){
					if(!self.video_pause){
						...
						self.form_video_show_overlay.find('img').attr('src', 'asset/images/'+self.options.player_color.replace('#', '')+'-main-video-play.png');						
						self.form_video_show_overlay.find('img').show();
					}else{
						self.video_pause = false;
						self.form_video_show[0].play();
						self.form_video_show_overlay.find('img').attr('src', 'asset/images/'+self.options.player_color.replace('#', '')+'-main-video-pause.png');						
					}
				});
				...
				self.processbar[0].addEventListener("mouseover", function(event){
					if (!self.draging)
					{
						if (self.options.skin != 2){
							...
							self.processbar_show_duration.css({
								'left': event.pageX-(parseInt(self.options.form_video_list_width)+100),
								'top': '-90px',
								'display': 'inline-block'
							});
						}
		...
		self.resizeFix = function(){
			...
			if (self.options.skin > 1)
			{
				...
				self.slide_video.css({
					'height': self.form_video_show.height()+self.form_video_text.outerHeight()+self.form_control.height()
				});
				self.main_form.css({
					'height': self.form_video_show.height()+self.form_video_text.outerHeight()+self.form_control.height()
				});
		...
		self.createOverLayVideo = function(){
			...
			if (self.options.skin > 1){
				self.form_video_show_overlay.css({'top': self.form_video_show.position().top});
			}
			...
			self.form_video.append(self.form_video_show_overlay);
			if(!self.options.auto_play || self.video_pause){
				self.form_video_show_overlay.find('img').attr('src', 'asset/images/'+self.options.player_color.replace('#', '')+'-main-video-play.png');						
				self.form_video_show_overlay.find('img').show();
			}
		};
		self.createScreenLoading = function()
		{
			...
			bg_load.css({
				...
				'height': self.form_video_show.height()+15,
				'opacity': 1,
			...
			container_load.css({
				...
				'top': self.form_video_show.position().top+(self.form_video_show.height()/2)-90,
				'text-align': 'center'
			});
			if (self.options.skin == 2){
				container_load.css({'left': self.main_form.position().left+(self.main_form.width()/2)-105});
			}
			if (self.options.skin == 3){
				container_load.css({'left': self.main_form.position().left+(self.main_form.width()/2)-96});
			}
		...
		self.resizeForm = function(){
			...
			if (self.options.skin == 1){
				jQuery(self).find('div.video-item').css({
					'width': '100px',
					'height': self.options.form_video_list_height,
				});
			}else{
			...
			if (self.options.skin == 1){
				jQuery(self).find('div.video-hover').css({
					'position':'absolute',
					'width': '100px',
					'height': '0px',
					'cursor': 'pointer',
